 ...
 * @return array
 */
function kgbtexas__body_classes( $classes ) {
...
add_filter( 'body_class', 'kgbtexas__body_classes' );

/**
 * Flush out the transients used in kgbtexas__categorized_blog.
 *
 * @return string
 */
function kgbtexas__category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return false;
	}
	// Like, beat it. Dig?
	delete_transient( 'kgbtexas__categories' );
}
add_action( 'delete_category', 'kgbtexas__category_transient_flusher' );
add_action( 'save_post', 'kgbtexas__category_transient_flusher' );
 ...
 * @return string
 */
function kgbtexas__content_more_link() {
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__( 'Read More', 'kgbtexas' ) . '...</a>';
}
add_filter( 'the_content_more_link', 'kgbtexas__content_more_link' );
 ...
 * @return string
 */
function kgbtexas__excerpt_more( $more ) {
	return sprintf( ' <a class="more-link" href="%1$s">%2$s</a>', get_permalink( get_the_ID() ), esc_html__( 'Read more...', 'kgbtexas' ) );
}
add_filter( 'excerpt_more', 'kgbtexas__excerpt_more' );
 ...
 * @return array
 */
function kgbtexas__custom_mime_types( $mimes ) {
	$mimes['svg']  = 'image/svg+xml';
	$mimes['svgz'] = 'image/svg+xml';
	return $mimes;
}
add_filter( 'upload_mimes', 'kgbtexas__custom_mime_types' );
...
add_action( 'wp_head', 'kgbtexas__display_customizer_header_scripts', 999 );
add_action( 'wp_footer', 'kgbtexas__display_customizer_footer_scripts', 999 );
...
add_shortcode( 'svg', 'kgbtexas__display_svg' );
